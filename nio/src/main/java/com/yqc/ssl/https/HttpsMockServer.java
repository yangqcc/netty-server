...
public class HttpsMockServer extends HttpsMockBase {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            try {
                ...
                doHttpsShakeHands(socket);
                executorService.execute(() -> doSocketTransport(socket));
    ...
    public static byte[] readBytes(int length) throws Exception {
        byte[] underpay = SocketUtils.readBytes(in, length);
        System.out.println("读取未解密消息:" + byte2hex(underpay));
        return DesCoder.decrypt(underpay, key);
    }

    public static void writeBytes(byte[] data) throws Exception {
        byte[] encrypted = DesCoder.encrypt(data, key);
        System.out.println("写入加密后消息:" + byte2hex(encrypted));
        SocketUtils.writeBytes(out, encrypted, encrypted.length);
    }
