 ...
 * 客户端:
 * <p>
 * 创建一个 Bootstrap 来初始化客户端 一个 NioEventLoopGroup 实例被分配给处理该事件的处理，这包括创建新的连接和处理入站和出站数据 创建一个
 * InetSocketAddress 以连接到服务器 连接好服务器之时，将安装一个 EchoClientHandler 在 pipeline 之后
 * Bootstrap.connect（）被调用连接到远程的 - 本例就是 echo(回声)服务器。
 *
 * @author yangqc
 */
